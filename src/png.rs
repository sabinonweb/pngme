...
impl Png {
    ...
    pub fn append_chunk(&mut self, c: Chunk) {
        self.chunks.push(c)
    }
...
// Takes bytes, runs through them and calculates lenfth and offset and creates a new Chunk.
impl TryFrom<&[u8]> for Png {
    type Error = Error;

    fn try_from(bytes: &[u8]) -> Result<Png> {
        if &bytes[0..8] != Png::STANDARD_HEADER {
            return Err("Invalid PNG header.".to_owned().into());
        }
